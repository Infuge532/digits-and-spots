...
├── digits-and-spots.md     — Full project & business brief (primary reference document)
└── README.md               — This file (project state, design system, task tracker)
```

> **For AI-assisted development sessions:** Upload both `digits-and-spots.md` (business brief) and `README.md` (project state) at the start of each new context, along with `css/style.css` and whichever HTML page you are actively working on. This gives the assistant full context on both the business and the codebase without needing to upload all files every time.
...
When continuing work in a new Claude (or other AI) context, upload these files at the start of the conversation:

| File | Why |
|---|---|
| `digits-and-spots.md` | Full business brief — practice details, page content, target audience, Kathleen's credentials, value propositions |
| `README.md` | This file — current project state, design system, pending tasks, version history |
| `css/style.css` | Design tokens and all shared component styles — essential for writing consistent new code |
| The HTML page you're working on | Only the file(s) relevant to the current task |

You do not need to upload all 7 HTML pages every session — just the ones being actively edited. The brief and README together give the assistant everything it needs to understand the project, the client, and the design system.

A good opening prompt for a new session:
> "I'm continuing work on the Digits & Spots website redesign. I've attached the project brief, the README, the shared stylesheet, and the page we're working on today. Please read all four files before we begin."
